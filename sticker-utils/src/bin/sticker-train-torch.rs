...
use sticker::torch::{CollectedTensors, RNNModel, Tagger, TensorCollector};
    ...
    Numberer<E::Encoding>: CborRead,
{
    ...
    let model = RNNModel::new_lstm(
        &vs,
        vectorizer.input_len() as i64,
        200,
        2,
        labels.len() as i64,
        0.40,
    ...
    let tagger = Tagger::random_weights(model, labels, vectorizer);
    ...
    E: Clone + Eq + Hash,
{
    ...
    for i in 0..tensors.labels.len() {
        ...
        let batch_perf = if is_training {
            tagger.train(Device::Cpu, tokens, labels)
        } else {
            tagger.train(Device::Cpu, tokens, labels)
            //tagger.validate(seq_lens, tokens, labels)
        };

        let n_tokens = i32::from(seq_lens.sum());
        loss += n_tokens as f32 * batch_perf as f32;
        //acc += n_tokens as f32 * batch_perf.accuracy;
        instances += n_tokens;

        progress.inc(1);
        progress.set_message(&format!("batch loss: {}", batch_perf));
    }
