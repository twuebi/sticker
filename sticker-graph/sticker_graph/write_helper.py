...
def create_graph(config, model, args):
    from tensorflow.contrib.summary.summary import import_event
    ...
    with tf.Graph().as_default(), tf.Session(config=tfconfig) as session:
        ...
        with summary_writer.as_default(), tf.contrib.summary.always_record_summaries():
            ...
            plc = tf.placeholder(name="metadata_input", dtype=tf.string, shape=[])

            metadata_import = import_event(plc,name="metadata_write")

            summary_init = tf.group(tf.contrib.summary.summary_writer_initializer_op(),
                                    name="summary_init")
            ...
            gr = tf.contrib.summary.graph(serialized_graph_tensor,
                                          step=0,
                                          name='graph_write')
